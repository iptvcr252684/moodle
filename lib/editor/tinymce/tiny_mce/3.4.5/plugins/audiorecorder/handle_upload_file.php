...
// It must have read/write permissions accessable your web server.
	//$uploads_dir = "uploads";
    $uploads_dir = $CFG->dataroot.'/'.$_GET['tempaudiofolder'];
...
//    }
    if( $upfile_size > $upfile_size_limit)  {
        ...
        if (file_exists('/usr/bin/ffmpeg')) {
            ...
            $toFile = $uploads_dir . '/' . $upfile_name . '.wav';
        }
        $moveResult = move_uploaded_file( $fromFile, $toFile );
        if( $moveResult ) {
            echo "SUCCESS - $upfile_name uploaded.\n";
            add_to_log($_GET['courseid'], "audiorecorder", "recorded", "/file.php/".$_GET['tempaudiofolder']."/$upfile_name", 'wav Size='. $upfile_size,0,$_GET['userid']);

            if (file_exists('/usr/bin/ffmpeg')) {
                $exec_string = '/usr/bin/ffmpeg -i '.$toFile.' '.$toFile.'.flv';
                exec($exec_string);
                add_to_log($_GET['courseid'], "audiorecorder", "recorded", "/file.php/".$_GET['tempaudiofolder']."/$upfile_name", 'flv',0, $_GET['userid']);
